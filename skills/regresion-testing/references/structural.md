...
extractSchema(obj):
  ...
  if obj is array:
    if empty             → "array"
    if non-empty         → [extractSchema(obj[0])]   // sample first element
  if obj is object:
...
1. **All values replaced with type name**: `"string"`, `"number"`, `"boolean"`, `"null"`, `"object"`
2. **Arrays with elements**: `["<type>"]` — sample first element's schema, wrap in array
3. **Empty arrays**: `"array"` — no type information available
4. **Nested objects**: Recurse — keys sorted alphabetically for consistency
5. **Mixed-type arrays**: Sample first element only — for heterogeneous arrays, consider splitting the cluster

### Implementation (fingerprint.js extension)

```js
/**
 * Extract structural schema from a JSON value.
 * All values replaced with their type name.
 */
export function extractSchema(obj) {
  ...
  return typeof obj  // "string", "number", "boolean", "undefined"
}
```

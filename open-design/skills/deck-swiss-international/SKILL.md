...
emoji: 🟦
description: 16-column grid, one saturated accent, and 22 locked layouts (Klein Blue,
  Lemon, Mint, Safety Orange).
zh_description: 16 列网格 + 单一饱和 accent + 22 个锁死版面 (Klein Blue / Lemon / Mint / Safety
  Orange)
en_description: 16-column grid, one saturated accent, and 22 locked layouts (Klein
  Blue, Lemon, Mint, Safety Orange).
category: slides
scenario: marketing
...
od:
  ...
  design_system:
    requires: false
  example_prompt: Use the Swiss International Deck template to turn my content into
    a 16-column-grid deck with one saturated accent and 22 locked layouts. Preserve
    the template's visual signature, use real content and data, and avoid lorem ipsum
